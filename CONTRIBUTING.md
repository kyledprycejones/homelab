...
This repo is orchestrated by both humans and AI agents (Planner/Engineer/Executor). To keep `main` stable we follow a predictable Git + directory workflow.
...
| `cluster/kubernetes/**`, `cluster/talos/**` | GitOps manifests; ConfigMaps like `cloudflared` are AI-editable |
| `infrastructure/proxmox/**`, `scripts/**` | Bootstrap + helper scripts (small, scoped edits only) |
...
| `cluster/kubernetes/platform/ingress/cloudflared/deployment.yaml`, destructive manifests | Human-owned architecture |
