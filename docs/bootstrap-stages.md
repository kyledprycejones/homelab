...
- **k3s** (alias: `cluster`): Prepare hosts, wipe stale known_hosts, install k3s server on the controller, join workers, wait for all nodes Ready. Fatal if nodes cannot join or k3s API is unreachable.
...
- **apps** (alias: `gitops`): Install Flux CLI on the controller, bootstrap Flux controllers, create GitRepository and Kustomization pointing at this repo/branch (`GIT_BRANCH` is the single source of truth). Fatal if Flux install/apply fails.
...
Fatal vs warnings:
- Fatal: SSH/auth/connectivity in precheck; k3s API or node join failures; NFS provisioner/PVC failures; Flux install/apply failures.
