...
- backlog_file: ai/backlog.md (for checkbox snapshot)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--backlog-file", default="ai/backlog.md", help="Backlog markdown to snapshot.")
...
def parse_backlog(path: Path, limit: int) -> List[dict]:
    ...
    for raw in path.read_text().splitlines():
        ...
        m = re.match(r"- \[( |x)\] (.+)", line)
        if m:
            items.append({"line": line, "checked": m.group(1) == "x"})
            if len(items) >= limit:
                break
    return items
...
def main() -> int:
    ...
    if status == "blocked_stage1":
        default_blocker = "Stage 2 (Biz2) is locked by mission.md; Stage 1 items must be completed first."
        if not stuck_on:
            stuck_on = default_blocker
        if not suggested_next_step:
            suggested_next_step = "Pick a Stage 1 backlog item (cloudflared, monitoring, ingress, sample apps) and advance that instead of Biz2."
